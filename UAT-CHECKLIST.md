...
| ultibackup.backup.restore.neg-checksum-failed | `language: en`; `Tester1` has a backup (from `ultibackup.backup.create`); its on-disk content YAML file under `plugins/UltiTools/backups/` is hand-edited to change one byte inside the YAML body while leaving the `# Checksum:` header line untouched, so the stored checksum no longer matches the (now-tampered) content. The byte changed is the last digit of the `expLevel:` value (for example `expLevel: 5` becomes `expLevel: 6`), which lies outside every item block: `ultibackup.gui.force-restore-confirm` below force-restores this same file and expects its items to read back, and since `UltiKits/UltiBackup#21` a byte changed inside an item block refuses the restore instead (that case is `ultibackup.gui.force-restore-confirm.neg-corrupt-items`) | Run `/backup restore 1` | Chat shows `⚠ Warning: Backup file checksum verification failed!` then `The file may have been modified or corrupted. Use force restore if you still want to proceed.` (both, and neither restores anything — `Tester1`'s CURRENT inventory is unchanged); this is `ultibackup.backup.restore`'s CHECKSUM_FAILED branch, distinct from its SUCCESS branch above | server | |
...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultibackup.gui.backup-browser | `language: en`; the pixel-layer test player has EXACTLY 46 to 90 backups with intact checksums (a count strictly in this range guarantees exactly 2 pages at 45 items/page — `Math.ceil(count / 45)`; 91 or more would produce 3+ pages and a different, untested page-indicator value; repeat `ultibackup.backup.create` as needed, or lower `max_backups_per_player` temporarily is NOT an option here since it would prune them; raise it instead); the browser is open via `ultibackup.backup.open` | Left-click the first (topmost, newest) backup item; separately (re-open, run this on a second attempt), shift+left-click a backup item; separately, right-click a backup item while holding `ultibackup.delete` or `ultibackup.admin`; separately, click the create-new-backup button (slot 47); separately, with the 46-90 backups present, click the next-page button (slot 53) then the previous-page button (slot 45) | Left-click restores that backup to the pixel-layer test player exactly as `ultibackup.backup.restore` does (chat `✔ Backup restored successfully!`, inventory contents replaced) and closes the inventory; shift+left-click opens `ultibackup.gui.backup-preview` for that backup WITHOUT restoring anything (the pixel-layer test player's inventory is unchanged immediately afterward); right-click deletes that backup (chat `✔ Backup deleted!`, the item's row is removed from the GUI on refresh, and its `backup_metadata` row plus its content YAML file are both gone); the create-new-backup button creates a fresh backup exactly as `ultibackup.backup.create` does and the GUI refreshes to show it; with the 46-90-backup fixture, the next-page button advances to page 2 — EXACTLY 2 total pages, so the indicator reads `Page 2 / 2` — showing backups 46 through the fixture's own count (NOT open-endedly "46th-and-later"), and the previous-page button returns to page 1 | pixel | BackupGUI |
...
| ultibackup.gui.force-restore-confirm.neg-corrupt-items | `language: en`; `plugins/UltiTools/pluginConfig/UltiBackup/lang/` moved aside before the server started (the console line quoted below is new in this version); `Tester1` is the disposable test player of `ultibackup.backup.create` above, holding `ultibackup.use` and `ultibackup.create`. Session-local environment change, restored in the last step: one scratch backup file's item data is made unreadable (step 3), with a copy of the original kept outside the backups folder | 1. From the console: `clear Tester1`, then `give Tester1 minecraft:diamond 3`. `Tester1` runs `/backup create`; note the file the console line `Created backup for Tester1: backups/<file>` names. 2. From the console: `clear Tester1`, then `give Tester1 minecraft:gold_ingot 9`. 3. Copy the file aside: `cp plugins/UltiTools/backups/<file> /tmp/<file>.bak`. Then edit the file's inventory block so its first `items:` line cannot be parsed: `sed -i '0,/^\( *\)items:$/s//\1items: [/' plugins/UltiTools/backups/<file>` (the inventory block is written first in the file, so this changes the inventory's own `items:` line and nothing else); `diff /tmp/<file>.bak plugins/UltiTools/backups/<file>` must print exactly that one changed line, else stop and record `blocked`. 4. From the console: `data get entity Tester1 Inventory`. 5. `Tester1` runs `/backup list`, then `/backup restore <n> force` with `<n>` the position of the backup made in step 1 (the newest), and clicks Confirm in the dialogue. 6. Read `Tester1`'s chat and the console, then `data get entity Tester1 Inventory` again. 7. Restore: `mv /tmp/<file>.bak plugins/UltiTools/backups/<file>`, then from the console `clear Tester1` | Step 6's chat shows `✖ Failed to restore backup!` (red) and NOT `⚠ Corrupted backup force-restored!`. The console shows a WARN line `Backup <id> was not restored to Tester1: its inventory data cannot be read, so nothing was changed` followed by the reader's stack trace, and no `Restored backup <id> to player Tester1` line. Step 6's inventory read is identical to step 4's: exactly one entry, `minecraft:gold_ingot` with count 9, and no `minecraft:diamond`. Record `fail` if the chat shows the force-restored line, or if the inventory read shows the gold ingots gone (an empty inventory is exactly `UltiKits/UltiBackup#21`: cleared, nothing restored, success reported). If step 3's `diff` shows anything other than the one line, or the chat shows `Failed to load backup file!` (the edit broke the whole file rather than the inventory block), record `blocked` with the diff and the chat attached | server | ForceRestoreConfirmPage |
