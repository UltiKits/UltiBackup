...
| ultibackup.gui.force-restore-confirm | Confirmation dialogue (extends the framework's `BaseConfirmationPage`, the only one of this module's three GUI classes to use it rather than a raw `InventoryHolder`) warning that the target backup failed checksum verification and that force-restoring risks item loss or corruption; Confirm performs the restore, Cancel aborts with no state change. A restore reads every part it will apply before it changes anything: when a part that is not empty cannot be read back into items, nothing is cleared or applied and the sender is told the restore failed (`UltiKits/UltiBackup#21`); a backup file that is not valid YAML, has no `inventory` section, is cut off before its last key (`expProgress`), or whose entries are not what the module writes (a part that is not text, `armor` without `offhand` or the reverse, `expLevel` missing or not an integer, `expProgress` not a number) is refused as a load failure; a part holding a slot outside the inventory it is restored to (36 storage, 4 armor, 27 ender chest slots) counts as unreadable; when `backup_exp` is on, a negative `expLevel` or an `expProgress` outside 0-1 refuses the restore the same way, before anything changes | gui | `ultibackup.backup.restore-force` (the `/backup restore <number> force` command path), OR left-clicking a backup item in `ultibackup.gui.backup-browser` whose checksum verification fails — that click path is `BackupListener#handleRestore` specifically, NOT the plain `/backup restore <number>` command: `BackupCommand#handleRestore` (a separate, identically-named private method on a different class) only sends the checksum-failure chat lines on a CHECKSUM_FAILED result and never opens this dialogue at all, so a checksum failure reached via the bare command (without `force`) does NOT route here | n/a | n/a | player | detailed | ForceRestoreConfirmPage#setupDialogContent, ForceRestoreConfirmPage#onConfirm, ForceRestoreConfirmPage#onCancel |
